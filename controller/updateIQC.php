...
if (isset($_FILES['fileToUpload']['tmp_name']) AND $_FILES['fileToUpload']['tmp_name']!="")	{
  ...
  $sheet = $objPHPExcel->getSheetByName('INSPECTION QUALITE DIM INSTRUM');
...
$oIQC->updateComments($_POST['id_tbljob']);
  ...
  for ($row = 15; $row <= $highestRow; $row++){
    ...
    $rowData = $sheet->rangeToArray('A'. $row.':P'.$row,
    NULL,
    TRUE,
    FALSE);
    ...
    if ($rowData[0][0]!="") {
      $oIQC = new AnnexeIQCModel($db);
      $IsIQC=$oIQC->existIQC($rowData[0][0]);


      $oIQC2 = new AnnexeIQCModel($db);

      $oIQC2->dim1=$rowData[0][3];
      $oIQC2->dim2=$rowData[0][4];
      $oIQC2->dim3=$rowData[0][5];
      $oIQC2->marquage=$rowData[0][7];
      $oIQC2->surface=$rowData[0][8];
      $oIQC2->grenaillage=$rowData[0][9];
      $oIQC2->revetement=$rowData[0][10];
      $oIQC2->protection=$rowData[0][11];
      $oIQC2->autre=$rowData[0][12];

      $oIQC2->observation=$rowData[0][13];

      $oIQC2->id_tech=$_COOKIE['id_user'];


      if ($IsIQC) {
        $oIQC2->updateIQC($rowData[0][0]);
      }
      else {
        $oIQC2->InsertIQC($rowData[0][0]);
      }
